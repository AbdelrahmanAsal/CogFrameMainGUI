...
//				"Primary user behaviour inactive distribution",
	...
	public void drawChart(int index) throws IOException {
...
//		} else if (index == 5) {
	...
	public JFreeChart getProbabilityDistributionChart(ArrayList<Double> data, String type) {
		...
		JFreeChart chart = ChartFactory.createXYLineChart("Primary user "+type+" behaviour", "Time (ms)",
	...
	public JFreeChart getSwitchingTimeChart() {
		...
		XYSeriesCollection xyseriescollection = new XYSeriesCollection();
		xyseriescollection.addSeries(dataset);
		JFreeChart chart = ChartFactory.createXYLineChart("CDF of switching time", "Time (ms)",
				"CDF", xyseriescollection, PlotOrientation.VERTICAL, false,
				true, false);
		return chart;
	}
	...
	public JFreeChart getLossRatioChart() { 
		...
		for(int packetID : dest.inPackets.keySet()){
			...
			dataset.add((time - minTime)/1000.0, 1 - (1.0 * rec / sent));
		}
		XYSeriesCollection xyseriescollection = new XYSeriesCollection();
		xyseriescollection.addSeries(dataset);
		JFreeChart chart = ChartFactory.createXYLineChart("Packets Loss Ratio", "Time (seconds)",
	...
	public JFreeChart getPacketDelayChart() { 
		...
		for(int packetID : dest.inPackets.keySet()){
			...
			dataset.add((time - minTime)/1000.0, totalDelay / totalCount);
		}
		XYSeriesCollection xyseriescollection = new XYSeriesCollection();
		xyseriescollection.addSeries(dataset);
		JFreeChart chart = ChartFactory.createXYLineChart("Packet Delay", "Time (seconds)",
				"Packet Delay (sec)", xyseriescollection, PlotOrientation.VERTICAL, false,
				true, false);
		return chart;
	}
